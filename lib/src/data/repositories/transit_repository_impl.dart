...
final class TransitRepositoryImpl implements TransitRepository {
  ...
  Future<BusLocationModel?> getBusLocation(String busId) async {
    ...
    try {
      ...
      if (error.response?.statusCode == 404 || error.response?.statusCode == 503) {
        return null;
      }
  ...
  String _normalizeArcGisUrl() {
    if (arcGisUrl.isEmpty) return '';
    return arcGisUrl.endsWith('/query')
        ? arcGisUrl.substring(0, arcGisUrl.length - '/query'.length)
        : arcGisUrl;
  }
  ...
  int _findNearestStopIndex(
    ...
    for (var i = 0; i < stops.length; i++) {
      final stop = stops[i];
      final distance =
          _haversineMeters(lat, lng, stop.lat, stop.lng);
